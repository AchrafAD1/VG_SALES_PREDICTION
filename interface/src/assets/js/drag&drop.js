...
var numbers = ['SELECT', 'Name', 'Year_of_Release', 'Genre', 'Publisher', 'MA_Sales', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count', 'Developer', 'Rating',
'FROM', 'Games'];
var reqPre = ['SELECT', 'Name', 'Year_of_Release', 'Genre', 'Publisher', 'MA_Sales', 'Critic_Score', 'Critic_Count', 'User_Score', 'User_Count', 'Developer', 'Rating',
'FROM', 'Games'];
...
btn.addEventListener('click', function(e) {
    ...
    if(!requests.includes('')) {
        ...
        if(found == requests.length){
            fetch(`http://127.0.0.1:5000/get_data/moroccan_sales`)
            .then(() =>  {
                showSuccessMessage();
            })
        }
...
showDataBtn.onclick = () => location.href = 'data_integration.html';
